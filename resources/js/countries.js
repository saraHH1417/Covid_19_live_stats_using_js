...
function createCountryList() {
        ...
        country_list.forEach((country, index) => {   
                ...
                document.getElementById(`${ul_list_id}`).innerHTML +=
                 `<li onclick='fetchData("${country.alpha3Code}" , "${country.englishShortName}")' id='${country.englishShortName}' >
...
// COUNTRY FILTER
//INPUT VALUE FIRES UP ONLY WHEN THE INPUT VALUE CHANGES
input.addEventListener("input" , function() {
        let value = input.value.toUpperCase();
        
        country_list.forEach(country => {
                if(country.englishShortName.toUpperCase().startsWith(value)) {
                        document.getElementById(country.englishShortName).classList.remove("hide");

                }else {
                        document.getElementById(country.englishShortName).classList.add("hide");
                }
        })
...
// CLOSE THE COUNTRY LIST WHEN USER CLICKS IN PART OF THE PAGE EXCEPT THE COUNTRY LIST DIV
document.addEventListener('mouseup', function(e) { //mouseup includes right click too. but click only includes left click
        if (!search_country_element.contains(e.target) && !change_country_btn.contains(e.target)) {
